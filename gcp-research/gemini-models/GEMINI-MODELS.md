...
3. **Production default for v2 is the Gemini 3.x series — judgment tier `gemini-3.5-flash` (GA 2026-05-19, callable on the `global` Vertex endpoint) and bulk/routing tier `gemini-3.1-flash-lite` — per the operator mandate of 2026-05-21 (DECISIONS.md D53, which supersedes D5).** `gemini-*-pro` is NOT accessible in ss-v2-prod (404, Preview allowlist not granted) and is not used; Gemini 3.x is served on the `global` endpoint, not us-central1. The 2.5 family is documented below as reference only and is **no longer the v2 production baseline**. (Historical note: the 2.5 family is GA, has stable function-calling semantics, a published rate card, and qualifies for provisioned throughput and supervised fine-tuning; the 3.x non-global rate card locks in **2026-07-01**.)
