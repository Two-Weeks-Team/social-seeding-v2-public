...
| **D28** | **Pricing**: **per campaign, the first 10,000 delivered views are free, then $0.01 per delivered view** ($10 effective CPM beyond the free tier, ROI-linked — only bills when a campaign breaks out past 10k) | user 2026-05-19 R5 · free-tier added 2026-06-02 | ✅ | Per-view metering pipeline (Pub/Sub event → BigQuery → billing) required; the per-campaign free-10k tier lowers adoption friction (every campaign starts free) and is outcome-aligned (revenue only on breakout campaigns). The compounding asset is the proprietary creator-performance + outreach/communication dataset (raises switching cost + feeds the sourcing/vetting/outreach flywheel). No per-seat or per-creator alternatives shipped day-1 |
