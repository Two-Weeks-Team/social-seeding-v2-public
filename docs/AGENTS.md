...
| 8 | **research** _(Phase 5)_ | Lead's website → structured sales analysis (Modal crawl + Kimi) → outreach angle | `crm.enrich`, `crm.account.upsert` | the `crm.enrich` analysis shape | Kimi (via the `crm.enrich` capability) + gemini-3.1-flash-lite | 0.1 | crawl fails / site has no usable content | the v1 CRM enrichment batch (already automated) |
