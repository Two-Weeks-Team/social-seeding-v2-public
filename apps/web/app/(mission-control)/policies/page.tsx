...
import { type WorkspacePolicy } from "@ss/contracts";

/**
 * W5 — Autonomy policy editor. Phase 1 ships the approveShortlist gate
 * (always_ask / auto / auto_unless with fitScoreLt) + budgets + level preset.
 * Other gates render disabled with a "Phase 2/3" hint so the user sees the
 * full shape without thinking they can use them yet.
 *
 * Save is a server action — no client JS for this form. Changes affect *new*
 * campaigns only; in-flight runs read the policy that was active at their
 * start (the workflow's step.run("plan") captures it).
 */
...
async function savePolicyAction(formData: FormData): Promise<void> {
  ...
  const PolicyForm = z.object({
    ...
    "gate.approveShortlist.mode": z.enum(["always_ask", "auto", "auto_unless"]),
  ...
  const next: WorkspacePolicy = {
    ...
    gates: {
      ...existing.gates,
      approveShortlist: {
        mode: f["gate.approveShortlist.mode"],
        escalateIf:
          f["gate.approveShortlist.mode"] === "auto_unless" && f["gate.approveShortlist.fitScoreLt"] !== undefined
        ...
        <Card>
          <CardBody>
              ...
              <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-md p-0.5 w-fit">
                {(["always_ask", "auto", "auto_unless"] as const).map((mode) => (
                  <label key={mode} className="cursor-pointer">
                    <input
                      type="radio"
                      name="gate.approveShortlist.mode"
                      value={mode}
                      defaultChecked={sl.mode === mode}
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-slate-500">
              {(["approveOutreachSend", "approveReplyResponse", "approveShipment", "approveStageAdvance"] as const).map((g) => (
                <div key={g} className="border border-dashed border-slate-200 rounded px-3 py-2 flex justify-between items-center">
                  ...
                  <Badge variant="slate" className="!text-[10px]">Phase 2/3</Badge>
                </div>
              ))}
            </div>
          </CardBody>
