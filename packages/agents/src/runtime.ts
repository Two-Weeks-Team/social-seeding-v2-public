 ...
 * `model.ts`). Model routing — Gemini 3.1 Pro for judgment, Gemini 3.1
...
export async function runAgent<I extends z.ZodTypeAny, O extends z.ZodTypeAny>(
  ...
  return ctx.trace.span(`agent:${def.id}`, "agent", { model: def.model, maxUsd: def.maxUsd }, async () => {
     ...
     * yet — we set `tool_choice: any` so Gemini 3.1 Pro emits a native tool_use
     ...
     * Live-demo lesson 2026-05-14: without this, Gemini 3.1 Pro with long
     ...
     * NOT as a bracket-prefixed pseudo-syntax — Gemini 3.1 Pro was observed (live-demo
