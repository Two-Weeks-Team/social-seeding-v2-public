 ...
 * the same self-check loop the live Gemini 3.5 Flash path will run.
 *
 * What we pin:
...
describe("conversationResponderAgent — unit", () => {
  ...
  it("agent definition: Gemini 3.5 Flash, curated tools = [outreach.judge, templates.render], maxUsd ≤ 1.0", () => {
    // Cap raised 0.5 → 1.0 over two live-demo iterations 2026-05-14
    // (see agent file comment). 4-6 Gemini 3.5 Flash turns reliably consume
    // ~$0.5-0.7; the ceiling has to budget for a self-check + revise.
    expect(conversationResponderAgent.model).toBe("gemini-3.5-flash");
