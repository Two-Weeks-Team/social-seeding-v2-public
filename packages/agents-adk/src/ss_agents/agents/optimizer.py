...
Behavior (per task brief + optimizer.spec.md §1):
    ...
    Gemini 2.5 Pro because reflection quality matters; cost cap = $0.10
...
Citations:
    D5    — Gemini 2.5 Pro (judgment-heavy reflection).
...
# Gemini 2.5 Pro per D5 / ARCHITECTURE.md §3 row 19 — reflection quality
# outweighs per-call cost on this Tier-2 nightly path.
DEFAULT_OPTIMIZER_MODEL = "gemini-2.5-pro"
...
class EvalResultRow(BaseModel):
    ...
    below) so the prompt size stays predictable for Gemini 2.5 Pro.
    """
