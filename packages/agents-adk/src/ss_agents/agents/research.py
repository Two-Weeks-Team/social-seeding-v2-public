...
Port of v2's `packages/agents/src/research.agent.ts` onto ADK + Gemini 3.5 Flash
...
Citations:
    D5    — Gemini 3.5 Flash baseline (production model for v2 research).
    ...
    ARCHITECTURE.md §3 row 9 — model = Gemini 3.5 Flash; tools =
...
# Gemini 3.5 Flash per DECISIONS.md D53 / GEMINI-MODELS §4 (Vertex AI v1
# baseline). Pricing wired into MODEL_PRICING in config.py.
DEFAULT_RESEARCH_MODEL = "gemini-3.5-flash"
...
research_agent_def: AgentDef[ResearchInput, ResearchOutput] = AgentDef(
    ...
    description=(
        ...
        "marketPosition, recentNews[≤10], sources[]. Gemini 3.5 Flash; "
