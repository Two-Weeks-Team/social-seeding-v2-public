...
Tournament shape (per task brief + spec §1 + §5 sequence):
    1. 5 candidate drafts produced in parallel from 5 angles
       ...
       Each angle is its own short Gemini 3.5 Flash call (single-angle prompt).
...
Citations:
    D5  — Gemini 3.5 Flash for judgment (drafter + judges share the Pro tier).
    ...
    ARCHITECTURE.md §3 row 3:
        outreach_writer | 1 | Gemini 3.5 Flash tournament | templates.list,
...
outreach_drafter_agent_def: AgentDef[_DrafterInput, OutreachDraft] = AgentDef(
    ...
    model="gemini-3.5-flash",  # D53 — judgment-grade prose for a 200-800 char body.
...
outreach_judge_agent_def: AgentDef[_JudgeInput, JudgeScoreCard] = AgentDef(
    ...
    model="gemini-3.5-flash",  # D53 — calibration of [0,1] needs judgment-tier reasoning.
