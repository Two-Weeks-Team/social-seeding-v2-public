...
Citations:
    D5  — Gemini 2.5 Pro (planning role — judgment over speed).
    ...
    ARCHITECTURE.md §3 row 1:
        sourcing | 1 | Gemini 2.5 Pro | rapidapi.tiktok_search,
...
# Gemini 2.5 Pro per D5 — planning role. The 2-4 query loop + blacklist
...
DEFAULT_SOURCING_MODEL = "gemini-2.5-pro"
...
# Gemini 2.5 Pro port because the planning loop is model-agnostic in shape.
...
sourcing_agent_def: AgentDef[SourcingInput, SourcingOutput] = AgentDef(
    ...
    # stub ↔ live via CAPABILITY_LAYER_MODE; the agent (Gemini 2.5 Pro) is
