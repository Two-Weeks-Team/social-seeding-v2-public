...
Citations:
    D5  — Gemini 3.5 Flash for creative planning (multimodal Pro chosen over
    ...
    ARCHITECTURE.md §3 row 14:
        creative | 1 | Gemini 3.5 Flash + Imagen 4 + Veo 3
...
def build_creative_system_prompt(payload: BaseModel) -> str:
    ...
    Composes the Gemini 3.5 Flash system prompt with:
...
creative_agent_def: AgentDef[CreativeInput, CreativeOutput] = AgentDef(
    ...
    description=(
        ...
        "creative.spec.md (D23 Tier-1 agent #14, D5 Gemini 3.5 Flash, D29 "
        "multimodal differentiator)."
    ),
    model="gemini-3.5-flash",  # D53 — judgment tier for sustained creative reasoning
