...
| 17 | **Judge 1-click demo login** | A **bypass** login: `GET /api/auth/judge-demo?token=…` mints an `ss_session` JWT directly (no Google OAuth / 2FA), so reviewers land on a populated Mission Control. The minted session carries **`demo: true`** and **every state-mutating route refuses it** (`POST /api/campaigns`, `/campaigns/intake`, `/approvals/[id]/resolve|sign-mandate|reject-mandate` → 403; MC **server actions** — resolve gate, pause/cancel campaign, save policy, create campaign/lead, generate report — bounce via `demoReadonlyGuard` with a visible read-only notice) — it is a **read-only tour**. Token is HMAC-stored (raw token never in DB), globally **usage-capped**, **time-boxed** (`JUDGE_DEMO_EXPIRES_AT`), and **kill-switchable** (`JUDGE_DEMO_ENABLED`). The demo email is **not** Gmail-connected, so `gmail.send` is structurally impossible. Real Google OAuth login (identity-only scopes) remains the production path and is also live. Landing data = public TikTok creator rankings + the real Wooliliwoo pilot's outcomes; email threads are the pilot's REAL correspondence with **all contact details redacted at seed time** (emails · phones · shipping-address lines → visible `[… removed — creator privacy]` labels; `scripts/demo/seed-threads-from-backend.ts`, redaction ON by default). Substance verbatim, contacts stripped — labeled redaction, not silent substitution. → tested in `apps/web/__tests__/judge-demo.test.ts` | Real multi-tenant user→workspace login (Auth.js v5); the demo bypass is removed/flipped off after judging. | **operator-deploy** (env: `JUDGE_DEMO_ENABLED/TOKEN/WORKSPACE_ID/EXPIRES_AT`) |
...
| 21 | **Agent Sessions (Scale, GA) — multi-turn persistence** | Live on the deployed Agent Runtime: `create_session` → 2-turn `stream_query(session_id=…)` where turn 2 ("how many did you find?") answered "I returned 1 creator" **from the conversation** (stateful, not stateless) → `get_session` shows **8 events persisted**. Session `1945195017685434368` on engine `2498295477225652224`. (Sessions auto-included with Agent Runtime.) | Standing per-operator session store. | **✓ demonstrated-live** (2026-06-03) |
| 22 | **Agent Registry (Govern, New) — live read** | `agent_engines.list()` returns our **3 registered, discoverable** reasoningEngines (`2498…` / `1587…` / `8794…`) — a real registry read, not a stub claim. | Native Agent Registry discovery across the fleet. | **✓ demonstrated-live** (2026-06-03) |
| 23 | **Agent Compliance — Cloud DLP / Sensitive Data Protection (Govern)** | Live `dlp.inspect_content` on an agent-authored outreach draft flagged **3 PII findings** (EMAIL_ADDRESS `maria.ops@2weeks.co` LIKELY, PERSON_NAME ×2) — the gate before any `external_send` (`gmail.send`) leaves the fleet; any finding ≥ POSSIBLE routes to redaction/human review. `dlp.googleapis.com` enabled on `ss-v2-prod`. | Inline DLP de-identify on every outbound agent message. | **✓ demonstrated-live** (2026-06-03) |
| 24 | **Agent Memory Bank — fleet-wide per-user recall (Scale, GA)** | Deepens GT2: the SAME deployed agent recalls a DIFFERENT remembered constraint **per brand-operator** with NO ER stated — `wooriliu-op`→13%→1 creator, `glowco-op`→8%→4 creators (scoped `CreateMemory` on bank `1587…`). Proves per-user Memory Bank scoping, not one global memory. | Per-workspace memory scopes across the 22-agent fleet. | **✓ demonstrated-live** (2026-06-03) |
| 25 | **Gemini 3.x thinking budget (Optimize/Models)** | `BuiltInPlanner(ThinkingConfig(thinking_level=high))` wired into the deployed agent (`ae_deploy/agent.py`) + redeployed to engine `2498…`; the coordinator's ceiling judgments get real reasoning tokens. Validated live: thinking_level=high → `thoughts_token_count=186` (correctly rejected a 12.9% creator vs a 13% floor). | Per-agent thinking budgets tuned by task stakes. | **✓ demonstrated-live** (2026-06-03) |
| 26 | **Agent Simulation (Optimize, New)** | A gemini-3.5-flash "simulated brand operator" generated 3 diverse scenarios (normal / tight-ER edge case / ambiguous brief), each driven against the **live** thinking-enabled agent in its own Session — a real simulated-user ⇄ live-agent loop (scenario 2 correctly applied a 12% ER floor). **Honest boundary:** the *managed* `evals.generate_conversation_scenarios` (DataFoundry) was invoked correctly (schema-valid, reaches the service) but returns **500 INTERNAL twice** (experimental, Google-side), so we use the direct-loop better-method. → `scripts/native-adoption/` | Managed Agent Simulation once the DataFoundry endpoint stabilises; the direct loop is the working path. | **✓ demonstrated-live** (direct loop; managed endpoint = Google-side 500) |
...
1. **"Agent Optimizer" → "Prompt Optimizer."** There is no GA "Agent Optimizer." The GA product is the
   **Prompt Optimizer (data-driven / VAPO)**. The local deterministic pass demonstrates the
