...
| 5 | **A2A v0.3 signed agent card (JWS ES256) + JWKS** | **✓ DEMONSTRATED LIVE (2026-05-24).** The live `ss-mcp-server` serves a **signed** card — `signatures[]` JWS (RFC 7515) over the JCS-canonicalized (RFC 8785) card, **ES256 (ECDSA P-256 / SHA-256)**, with the matching public key at `/.well-known/jwks.json` (`kid: ss-agent-card-prod`), plus `securitySchemes` (oidc/oauth/mutualTLS) + `additionalInterfaces`. Signing key held in Secret Manager. → `curl …/.well-known/agent.json \| jq '.signatures'` + `curl …/.well-known/jwks.json`. (Earlier the live card was an unsigned stub due to a path bug — fixed PR #24.) | Promote the dev signing key to a **Cloud KMS**-held key + key rotation. | **✓ demonstrated-live** (KMS rotation is the prod refinement) |
