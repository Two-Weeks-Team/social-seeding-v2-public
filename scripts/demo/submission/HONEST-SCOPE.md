...
| 21 | **Agent Sessions (Scale, GA) — multi-turn persistence** | Live on the deployed Agent Engine: `create_session` → 2-turn `stream_query(session_id=…)` where turn 2 ("how many did you find?") answered "I returned 1 creator" **from the conversation** (stateful, not stateless) → `get_session` shows **8 events persisted**. Session `1945195017685434368` on engine `2498295477225652224`. (Sessions auto-included with Agent Engine.) | Standing per-operator session store. | **✓ demonstrated-live** (2026-06-03) |
