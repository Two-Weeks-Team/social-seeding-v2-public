...
- **8× cost reduction at parity.** Same golden set, same eval criteria, same outreach quality (`response_match_v2` + `spam_score`), shipped on Gemini 3.1 Pro / Flash-Lite routing where v1 used Claude exclusively. Per-campaign cost dropped from **$0.42** (measured n=20 on Claude) to **~$0.05** (measured on the Gemini 3.1 mix with Flash-Lite bulk-routing).
